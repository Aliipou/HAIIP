...
def sensor_timeseries(
    ...
    fig.update_layout(**PLOT_LAYOUT, height=height, title=dict(text=label, font=dict(size=13)))
...
def multi_sensor_chart(
    ...
    fig = go.Figure()
    color_cycle = [COLORS["primary"], COLORS["success"], COLORS["warning"],
                   COLORS["danger"], "#A78BFA", "#F472B6"]
    ...
    for i, (name, values) in enumerate(sensor_data.items()):
        fig.add_trace(go.Scatter(
            ...
            mode="lines",
            name=name,
            line=dict(color=color_cycle[i % len(color_cycle)], width=1.8),
        ))

    fig.update_layout(**PLOT_LAYOUT, height=height, title=dict(text=title, font=dict(size=13)))
    st.plotly_chart(fig, use_container_width=True)
...
def anomaly_score_gauge(score: float, title: str = "Anomaly Score") -> None:
    ...
    color = (
        COLORS["success"] if score < 0.4
        else COLORS["warning"] if score < 0.7
        else COLORS["danger"]
    )
...
def rul_bar_chart(machines: list[str], rul_values: list[int], height: int = 250) -> None:
    ...
    colors = [
        COLORS["danger"] if v < 50
        else COLORS["warning"] if v < 150
        else COLORS["success"]
        for v in rul_values
    ]
    ...
    fig.update_layout(
        **PLOT_LAYOUT,
        height=height,
        title=dict(text="Remaining Useful Life (cycles)", font=dict(size=13)),
...
def failure_mode_pie(labels: list[str], values: list[int]) -> None:
    ...
    colors = [COLORS["danger"], COLORS["warning"], COLORS["primary"],
              "#A78BFA", "#F472B6", COLORS["muted"]]
    fig = go.Figure(go.Pie(
        labels=labels, values=values,
        hole=0.55,
        marker=dict(colors=colors[:len(labels)], line=dict(color=COLORS["surface"], width=2)),
    ...
    fig.update_layout(
        ...
        title=dict(text="Failure Mode Distribution", font=dict(size=13)),
    )
    st.plotly_chart(fig, use_container_width=True)
...
def drift_heatmap(feature_names: list[str], psi_values: list[float]) -> None:
    ...
    colors = [
        COLORS["success"] if v < 0.1
        else COLORS["warning"] if v < 0.2
        else COLORS["danger"]
        for v in psi_values
    ]
    fig = go.Figure(go.Bar(
    ...
    fig.add_vline(x=0.2, line_dash="dash", line_color=COLORS["danger"],
                  annotation_text="Drift threshold", annotation_position="top right")
    fig.update_layout(
        **PLOT_LAYOUT,
        height=max(200, len(feature_names) * 40 + 80),
        title=dict(text="Feature Drift (PSI)", font=dict(size=13)),
        showlegend=False,
    )
    st.plotly_chart(fig, use_container_width=True)
