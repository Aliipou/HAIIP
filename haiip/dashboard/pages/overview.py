...
from haiip.dashboard.components.charts import failure_mode_pie, confidence_histogram
from haiip.dashboard.components.demo_data import (
    demo_alerts, demo_kpis, demo_machines, demo_predictions,
)
from haiip.dashboard.components.theme import kpi_card, section_title, status_dot, badge
...
def render() -> None:
    ...
    with chart_l:
        ...
        if predictions:
            ...
            failure_values = [counts.get(k, 0) for k in
    ...
    with chart_r:
        confs = [p.get("confidence", 0.9) for p in predictions] if predictions else \
                [round(0.7 + i * 0.003, 3) for i in range(100)]
