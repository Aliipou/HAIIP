...
SEV_BORDER = {"critical": "#FF4444", "high": "#FF8C00", "medium": "#FFB400", "low": "#2D3748"}
...
def render() -> None:
    ...
    alerts = sorted(alerts, key=lambda a: (SEV_ORDER.get(a.get("severity", "low"), 3),
                                           a.get("created_at", "")))

    # ── Summary badges ────────────────────────────────────────────────────────
    all_alerts = demo_alerts() if is_demo() else alerts
    s_counts = {s: sum(1 for a in all_alerts if a.get("severity") == s)
    ...
    with st.expander("➕ Create manual alert"):
        ...
        if submitted and machine_id and title:
            if is_demo():
                ...
                resp = api_post("/api/v1/alerts", {
                    "machine_id": machine_id, "severity": severity,
...
def _acknowledge(alert_id: str) -> None:
    if is_demo():
        st.success(f"✅ Alert acknowledged (demo mode).")
        return
