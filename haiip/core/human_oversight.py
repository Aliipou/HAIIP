...
class HumanOversightEngine:
    ...
    def compute_metrics(self) -> OversightMetrics:
        ...
        human_labels = np.array(
            [e.human_label if (e.human_overrode and e.human_label) else e.ai_label for e in events]
        )
        ...
        # ── HIR by action ──
        action_categories = {e.action_category for e in events}
        hir_by_action: dict[str, float] = {}
        for cat in action_categories:
            cat_events = [e for e in events if e.action_category == cat]
            if cat_events:
                hir_by_action[cat] = sum(1 for e in cat_events if e.human_reviewed) / len(
                    cat_events
                )
        ...
        human_costs = sum(
            e.expected_cost_human
            if (e.human_overrode and e.expected_cost_human is not None)
            else e.expected_cost_ai
            for e in events
        )
        risk_reduction_pct = (ai_costs - human_costs) / ai_costs * 100.0 if ai_costs > 0 else 0.0
