...
class RealisticFederatedScenario:
    ...
    def generate_partitions(self, df: pd.DataFrame) -> dict[str, pd.DataFrame]:
        ...
        for node_id in NODES:
            ...
            other_idx = ~failure_rows.index.isin(dom_rows.index) & ~failure_rows.index.isin(
                sec_rows.index
            )
            other_rows = (
                failure_rows[other_idx] if not failure_rows.empty else failure_rows.iloc[:0]
            )
            ...
            chosen = pd.concat(
                [
                    _sample(dom_rows, dom_n),
                    _sample(sec_rows, sec_n),
                    _sample(other_rows, other_n),
                ],
                ignore_index=True,
            )
            ...
            if violations:
                logger.warning(
                    "federated_assumption_violations node=%s violations=%s",
                    node_id,
                    violations,
                )
