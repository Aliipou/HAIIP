...
class OperatorSimulationModel:
    ...
    def simulate_operator_decision(
        ...
        accept_prob = max(0.05, min(0.99,
            base_rate * fatigue_mult - fp_penalty + explanation_boost
        ))
...
def generate_oversight_report(
    ...
    low_confidence = [
        k for k, v in confidence_report.items()
        if v["confidence"] in ("LOW", "VERY LOW", "NONE")
    ]

    if low_confidence:
        logger.warning(
            "oversight_report_low_confidence_assumptions=%s "
            "field_study_required=True",
            low_confidence,
        )
