...
try:
    ...
    try:
        from haiip_msgs.msg import AIPrediction, EconomicDecision as EconDec
        _MSG = "haiip_msgs"
    except ImportError:
        from std_msgs.msg import String as AIPrediction, String as EconDec  # type: ignore
        _MSG = "std_msgs"
    _ROS2 = True
except ImportError:
    _ROS2 = False
        ...
        def __init__(self) -> None:
            ...
            for mid in list(self.get_parameter("machine_ids").value):
                self._pubs[mid] = self.create_publisher(
                    EconDec, f"/haiip/decision/{mid}", _QOS
                )
                self.create_subscription(
                    AIPrediction, f"/haiip/ai/{mid}",
                    lambda msg, m=mid: self._on_ai(msg, m), _QOS,
                )
